 ...
 * Moved to /api/ directory to avoid access restrictions on /includes/
 */
...
switch ($action) {
    ...
    case 'get_roles':
        ...
        // Unwrap nested response: api->request returns {success, data: {success, roles}}
        // Frontend expects: {success, roles}
        if (!empty($result['success']) && !empty($result['data']['roles'])) {
            echo json_encode(['success' => true, 'roles' => $result['data']['roles']]);
        } else {
            $error = $result['data']['error'] ?? $result['error'] ?? 'Failed to load roles';
    ...
    case 'assign_role':
        // Assign a Discord role to a user (used when application is accepted)
        $data = json_decode(file_get_contents('php://input'), true);
        $userId = $data['user_id'] ?? '';
        $roleId = $data['role_id'] ?? '';
        if (!$userId || !$roleId) {
            echo json_encode(['success' => false, 'error' => 'Missing user_id or role_id']);
            break;
        }
        $result = $api->assignRole($userId, $roleId);
        echo json_encode($result);
        break;
