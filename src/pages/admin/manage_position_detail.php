...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['assign_user_id'])) {
        ...
        $date_assigned = !empty($_POST['date_assigned']) ? $_POST['date_assigned'] : date('Y-m-d');

        $check_stmt = $pdo->prepare("SELECT id FROM user_positions WHERE user_id = ? AND position_id = ?");
        $check_stmt->execute([$user_id_to_assign, $position_id]);

        if ($check_stmt->rowCount() == 0) {
            $stmt_assign = $pdo->prepare("INSERT INTO user_positions (user_id, position_id, date_assigned, assigned_by) VALUES (?, ?, ?, ?)");
            $stmt_assign->execute([$user_id_to_assign, $position_id, $date_assigned, $_SESSION['user']['id']]);
            echo "<script>
                document.addEventListener('DOMContentLoaded', function() {
                    Swal.fire({icon: 'success', title: 'Assigned!', text: 'Position has been assigned.', timer: 1500, showConfirmButton: false})
                    .then(() => { window.location.href = 'manage_position_detail?id=" . $position_id . "'; });
                });
            </script>";
        } else {
            echo "<script>
                document.addEventListener('DOMContentLoaded', function() {
                    Swal.fire({icon: 'error', title: 'Error', text: 'This member already has this position.'});
                });
            </script>";
        }
    } elseif (isset($_POST['remove_user_id'])) {
        $user_id_to_remove = (int) $_POST['remove_user_id'];
        $stmt_remove = $pdo->prepare("DELETE FROM user_positions WHERE user_id = ? AND position_id = ?");
        $stmt_remove->execute([$user_id_to_remove, $position_id]);
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({icon: 'success', title: 'Removed!', text: 'Position has been removed from this member.', timer: 1500, showConfirmButton: false})
                .then(() => { window.location.href = 'manage_position_detail?id=" . $position_id . "'; });
            });
        </script>";
    }
...
// Fetch Users with this position
$stmt_users = $pdo->prepare("
    SELECT u.id, u.personaname, u.avatar, u.rank, u.status, up.date_assigned 
    FROM user_positions up 
    INNER JOIN users u ON up.user_id = u.id 
    WHERE up.position_id = ? 
    ORDER BY up.date_assigned DESC, u.personaname ASC
");
$stmt_users->execute([$position_id]);
$players = $stmt_users->fetchAll(PDO::FETCH_ASSOC);

// Fetch Users NOT having this position for the assignment dropdown
$stmt_other_users = $pdo->prepare("
    SELECT id, personaname, `rank` 
    FROM users 
    WHERE id NOT IN (SELECT user_id FROM user_positions WHERE position_id = ?) 
    ORDER BY personaname ASC
");
$stmt_other_users->execute([$position_id]);
...
<div style="max-width: 1400px; margin: 0 auto; padding: 20px;">
    ...
    <div class="header-actions">
        <a href="manage_positions" class="btn-back">
    ...
    <div class="admin-card">
        <div
            ...
            <h3 style="margin: 0; padding-bottom: 0; border-bottom: none;">Assigned Personnel <span
            ...
            <form method="POST" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <input type="date" name="date_assigned" value="<?php echo date('Y-m-d'); ?>"
        ...
        <?php if (empty($players)): ?>
            <div style="text-align: center; padding: 40px; color: #777;">
                ...
                <p>No personnel have been assigned this position yet.</p>
            </div>
        <?php else: ?>
                    ...
                    <div class="player-card">
                        <div class="card-top">
                            <form method="POST" style="position: absolute; top: 10px; right: 10px; z-index: 10;"
                                onsubmit="return confirm('Remove this position from the member?');">
                        ...
                        <div class="card-middle">
                            ...
                            <p class="player-role">Assigned:
                                <?php echo date('M d, Y', strtotime($member['date_assigned'])); ?>
                            </p>
                        </div>
