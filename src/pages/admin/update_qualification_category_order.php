...
try {
    ...
    $stmt = $pdo->prepare("UPDATE qualification_categories SET order_index = ? WHERE id = ?");
    foreach ($data['order'] as $index => $catId) {
    ...
    if ($pdo->inTransaction())
        $pdo->rollBack();
    error_log("Qualification Category Reorder Error: " . $e->getMessage());
