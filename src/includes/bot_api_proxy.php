...
switch ($action) {
    ...
    case 'get_server_leave':
        $result = $api->request('/settings/server-leave', [], 'GET');
        echo json_encode($result);
        break;

    case 'set_server_leave':
        $data = json_decode(file_get_contents('php://input'), true);
        $result = $api->request('/settings/server-leave', $data, 'POST');
        echo json_encode($result);
        break;

    case 'get_voice_logs':
        $result = $api->request('/settings/voice-logs', [], 'GET');
        echo json_encode($result);
        break;

    case 'set_voice_logs':
        $data = json_decode(file_get_contents('php://input'), true);
        $result = $api->request('/settings/voice-logs', $data, 'POST');
        echo json_encode($result);
        break;
